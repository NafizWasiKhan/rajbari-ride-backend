document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateRideStatus(status, details = {}) {
        ...
        if (isActive && statusContainer) {
            ...
            // Critical Fix: Ensure sheet is visible when status updates!
            try {
                // If sheet is collapsed (low on screen), snap it up
                const sheet = document.getElementById('bottom-sheet');
                if (sheet) {
                    const rect = sheet.getBoundingClientRect();
                    const height = window.innerHeight;
                    // If top is > 80% of screen height, it's collapsed. Pull it to 50%
                    if (rect.top > height * 0.8) {
                        if (typeof updateSheetHeight === 'function') {
                            // Try to use internal function if exposed or just set style manually
                            // Since updateSheetHeight is local, we check global exposure or manually set
                            // We will expose it globally in the bottom logic, but for now manual override:
                            sheet.style.top = '50vh';
            ...
            } catch (e) { console.warn("Sheet auto-open failed", e); }
        }
    ...
    // passengerPollInterval is already declared at top scope
    ...
    // --- Super Safety Net (Fix for "Not Visible" Issue) ---
    setTimeout(() => {
        console.log("[Safety] Checking UI visibility...");

        // 1. Force Bottom Sheet Open
        const sheet = document.getElementById('bottom-sheet');
        if (sheet) {
            const rect = sheet.getBoundingClientRect();
            // If sheet is too low (effectively hidden), pull it up
            if (rect.top > window.innerHeight * 0.8) {
                console.warn("[Safety] Sheet stuck at bottom. Forcing to 50vh.");
                sheet.style.top = '50vh';
                sheet.style.height = '50vh';
            }
        }

        // 2. Force Dashboard Visibility
        const passengerUI = document.getElementById('passenger-dashboard');
        const driverUI = document.getElementById('driver-dashboard');

        const pVisible = passengerUI && window.getComputedStyle(passengerUI).display !== 'none';
        const dVisible = driverUI && window.getComputedStyle(driverUI).display !== 'none';

        if (!pVisible && !dVisible) {
            console.warn("[Safety] Dashboards hidden. Forcing Passenger UI.");
            if (passengerUI) passengerUI.style.display = 'block';
            if (driverUI) driverUI.style.display = 'none';
        }

        // 3. Force Map Refresh (Fix gray/blank map)
